...
def contieneLasVocales(cadena):
    ...
    for c in cadena1:
        if c == "a" or c== "á" or c=="A":      # dependiendo de cuantas vocales tenga la palabra, se vas acumulando
            xA += 1
        if c == "e" or c=="é" or c=="E":
            xE += 1
        if c == "i" or c=="í" or c=="I":
            xI += 1
        if c == "o" or c=="ó" or c=="O":
            xO += 1
        if c == "u" or c=="ú" or c=="U":
            xU += 1
...
def traducirTelefono(numero):
    ...
    for letra in letrasTraducidas:
        ...
        for numero in separador:
            if numero in letra:
                numerosTraducidos += "-"
    return numerosTraducidos  # aqui se imprimen los numero ya traducidos mas el "01800-" que viene por default
